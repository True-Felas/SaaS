...
public class PlanController {
	...
	public ResponseEntity<List<PlanDTO>> obtenerPlanes() {
		List<PlanDTO> planes = planService.obtenerPlanesActivos().stream().map(mapper::toPlanDTO).toList();
	...
	public ResponseEntity<PlanDTO> obtenerPlan(@PathVariable Long id) {
		return planService.obtenerPorId(id).map(mapper::toPlanDTO).map(ResponseEntity::ok)
